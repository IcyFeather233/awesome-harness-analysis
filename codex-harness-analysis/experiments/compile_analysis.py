...
CLAIMS = [
    ...
    claim("C-009", "命令执行先经过 approval/exec policy，再进入 sandbox；在 approval_policy=never 时，模型请求 require_escalated 会在创建进程前被拒绝。", "exec tool, never/read-only", ["S-012", "S-013", "X-004", "X-007"], ["permissions_safety", "sandbox_execution"], "请求 touch 文件并声明 require_escalated；X-004 返回 policy error，X-007 证实目录与文件哈希未变。"),
...
OBSERVATIONS = [
    ...
    evidence("S-014", "S", "SandboxManager 将 permission profile 转为 runtime permissions，并把命令变换到 platform sandbox executor。", "codex-rs/core/src/exec.rs:SandboxManager", ["C-010"], path="codex-rs/core/src/exec.rs", line=117, end=413, symbol="SandboxManager"),
...
MODULE_FILES = {
    ...
    "tools_extensions": ["codex-rs/core/src/tools/spec_plan.rs", "codex-rs/core/src/tools/router.rs", "codex-rs/core/src/tools/registry.rs"],
    "permissions_safety": ["codex-rs/core/src/exec_policy.rs", "codex-rs/core/src/tools/sandboxing.rs"],
...
> 冻结对象：`rust-v0.144.5`，peeled commit `{COMMIT}`。本报告分析的是 Codex harness，而不是模型本身，也不把代码形状推断出的理由冒充作者意图。
...
REPORTS["00-design-space-and-running-example.md"] = """
...
REPORTS["01-scope-and-method.md"] = f"""
# 范围与方法

## 冻结快照

- Repository: `openai/codex`
- Annotated tag: `rust-v0.144.5`（tag object `{TAG_OBJECT}`）
...
REPORTS["02-interfaces-lifecycle.md"] = f"""
# 接口与生命周期

Codex 的接口层比“CLI 包一层 core”更丰富，但共享同一个 thread/session 概念。

`codex-rs/cli/src/main.rs` 的 `Subcommand` 将 `exec`、`mcp-server`、`app-server` 等模式显式分派；无 subcommand 时进入 TUI。[源码]({source_url('codex-rs/cli/src/main.rs', 124)}) [S: `S-001`]
...
外层 `RegularTask` 管一个用户 turn：发开始事件、调用 `run_turn`、处理排队输入。[源码]({source_url('codex-rs/core/src/tasks/regular.rs', 37)}) 内层 `run_turn` 才是 agent loop：检查 compaction、刷新 world state/skills/hooks、构造 prompt、采样、dispatch tools、判断 follow-up，最后执行 stop hooks 和结束事件。[源码]({source_url('codex-rs/core/src/session/turn.rs', 224)}) [S: `S-002`, `S-003`]

`OutputItemDone` 遇到 function/custom tool 时不会同步阻塞整个 stream parser，而是创建 tool future；Response completed 后再综合 `needs_follow_up`、pending input 与 compaction 条件。只读/parallel-capable handler 可共享读锁，exclusive handler 获取写锁。[源码]({source_url('codex-rs/core/src/tools/parallel.rs', 42)}) [S: `S-025`]
...
REPORTS["04-context-memory-compaction.md"] = f"""
# 上下文、记忆与压缩
...
`ContextManager` 按 oldest-first 保留模型可见 items、token info、reference item 和 world-state baseline；append 时只记录可进入 API 的 messages，并做 prompt normalization / modality filtering。[源码]({source_url('codex-rs/core/src/context_manager/history.rs', 36)}) [S: `S-005`]

另一个状态面是 typed world state：AGENTS instructions、environment/subagents、apps、plugins、extension contributors 等可形成 snapshot 或 RFC 7386 风格 diff。[源码]({source_url('codex-rs/core/src/context/world_state/mod.rs', 206)}) [S: `S-006`] 这让“当前环境变化”不必反复复制整个历史，但也意味着 history 与 baseline 的同步是 correctness 边界。[I: `I-001`]

每一轮的 `StepContext` 绑定当前 cwd、policy、model、tools 等请求级语义；tool invocation 保存精确 StepContext，避免并发工具读取到下一 turn 的配置。[S: `S-004`, `S-010`]

## Compaction 不是单一路径

源码至少有三种触发：turn 前 token 超限、tool follow-up 中超限、模型兼容性/窗口下调。选择器再区分 remote v2、remote v1 与 local compact；compaction 会安装替换后的 history 并重算 token usage。[源码]({source_url('codex-rs/core/src/session/turn.rs', 798)}) [S: `S-007`]

本轮没有构造足够长的 context，所以图中的 compaction 是静态虚线，不能宣称在默认任务中出现频率。

## 长期 memory

`memories` 默认关闭。启用后 Phase 1 从 eligible rollout 提取结构化记忆并 redaction，Phase 2 在全局锁/租约下由专用 consolidation agent 合并到 durable files。[文档]({source_url('codex-rs/memories/README.md', 1)}) [S: `S-020`] 它不是普通 turn 的同步 message append，也不等同于 session rollout。
"""

REPORTS["05-models-tools-extensions.md"] = f"""
# 模型、工具与扩展
...
`ModelProviderInfo` 把 base URL、auth、wire transport、request/stream retry 与 remote compaction capability 收拢到 provider abstraction；`ModelClientSession` 保存 turn 内稳定 transport 状态和 WebSocket fallback。[源码]({source_url('codex-rs/model-provider-info/src/lib.rs', 90)}) [S: `S-008`, `S-024`]

请求不是直接由 history 序列化：prompt 还包含 base instructions、output schema 和当前模型可见 tool specs。[源码]({source_url('codex-rs/core/src/session/turn.rs', 1084)}) [S: `S-004`]

## Registry != exposure

`ToolSpecPlan` 同时生成 handler registry 与 model-visible specs；hidden/deferred tools 可以存在于 registry 却不出现在请求中。内置 shell、utility、collaboration、MCP runtime、extensions、dynamic 与 hosted tools 在当前 StepContext 下合成。[源码]({source_url('codex-rs/core/src/tools/spec_plan.rs', 158)}) [S: `S-009`]
...
`ToolRouter` 解析 function/custom/namespace call，构造绑定 StepContext 的 invocation；registry 查找 handler、发 telemetry、运行 pre-tool hooks，再 dispatch。[源码]({source_url('codex-rs/core/src/tools/router.rs', 112)}) [S: `S-010`, `S-011`]
...
REPORTS["06-permissions-sandbox-workspace.md"] = f"""
...
`ExecPolicy` 先把复合命令拆成 segments，综合 rules 与 fallback。只有每个 segment 都显式 allow 才能绕过额外 sandbox 决策；危险/未知命令在不同 approval mode 下会 forbid、prompt 或依赖 sandbox。[源码]({source_url('codex-rs/core/src/exec_policy.rs', 169)}) [S: `S-012`]
...
通过 policy 后，`SandboxManager` 将 filesystem/network permission profile 转换为平台执行后端：Linux、macOS、Windows 有不同 transform 与 fallback。[源码]({source_url('codex-rs/core/src/exec.rs', 117)}) [D: `D-002`] [S: `S-014`]
...
REPORTS["07-subagents-delegation.md"] = f"""
...
一个 root tree 共享 `AgentControl`、registry/limiter/rollout budget；`CodexDelegate` 创建独立 child Codex，拥有自己的 channels、session、context 与 history，同时继承 effective config、provider、approval、sandbox、cwd、MCP、skills 和 tools。[源码]({source_url('codex-rs/core/src/codex_delegate.rs', 70)}) [S: `S-016`]

历史继承不是单一布尔值：full-history fork 保留 user/developer/final assistant 和 metadata；truncated fork 强制 child 重建上下文。[S: `S-017`]

默认配置中 V1 `multi_agent` 开启，V2 关闭；默认最大 child threads 为 6，V2 并发 session 为 4，深度为 1。[源码]({source_url('codex-rs/core/src/config/mod.rs', 203)}) [S: `S-015`] 这些数字是版本/配置条件，不应画成 harness 的普遍常数。
...
REPORTS["08-sessions-persistence-recovery.md"] = f"""
# Session、持久化与恢复
...
`ThreadStore` 是 storage-neutral interface，覆盖 create/resume/append/persist/flush/shutdown/load/list/archive/delete；`LiveThread` 是活动持久化边界，resume 时加载 history，append 时先做 persistence filtering 和 metadata patch。[源码]({source_url('codex-rs/thread-store/src/live_thread.rs', 91)}) [D: `D-004`] [S: `S-018`]

local store 的 rollout 文件采用 `rollout-<timestamp>-<thread-id>.jsonl`。后台 writer 在 I/O error 时保留 pending suffix，turn complete 前 task 会 flush，并把失败作为 warning/retry surface。[源码]({source_url('codex-rs/rollout/src/recorder.rs', 1511)}) [S: `S-019`, `S-022`]
...
REPORTS["09-observability.md"] = f"""
# 可观测性与评估

Codex 有两套互补表面：用户/host 可消费的 thread、turn、item、tool events；以及 OTel logs/traces/metrics。

任务层创建 turn span，带 conversation/turn/model/token 等字段；tool runtime 发 `codex.tool_call` log，含 trace id、conversation id、turn id、tool/call id 与 duration。[源码]({source_url('codex-rs/core/src/tasks/mod.rs', 384)}) [S: `S-021`]

OTel 配置默认不记录 user prompts，exporter 可单独关闭，shutdown 必须显式完成。[文档]({source_url('codex-rs/otel/README.md', 1)}) [D: `D-006`]
...
REPORTS["10-design-decisions.md"] = """
# 设计决策与权衡
...
REPORTS["11-runtime-experiments.md"] = """
# 运行实验
...
REPORTS["12-failure-modes-open-questions.md"] = """
# 失败模式与开放问题
...
REPORTS["13-coverage-reproducibility.md"] = f"""
# 覆盖率与复现
...
def compile_bundle() -> None:
    manifest = {
        ...
        "tooling": {"skill": "harness-analysis", "skill_commit": "b44224f0c2666d6461e7be4fb4af840ce67e05cd", "skill_schema_version": "1.0", "image_skill": "gpt-image-2"},
    }
